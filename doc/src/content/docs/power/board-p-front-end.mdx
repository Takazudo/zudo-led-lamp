...
description: The USB-PD 15V/3A sink front end for this lamp — adapted from the zudo-pd project's reusable Board A core (STUSB4500 + USB-C receptacle + load switch + NVM/debug pads), audited against primary datasheets rather than copied verbatim.
sidebar_position: 10
---
...
downstream — the LED power stage and/or control board being researched in parallel
(see the sibling `led-power` and `control` doc sections once those land; the exact
board split is locked by the prototype-architecture decision, not by this page).
...
number was re-verified against the local JLCPCB parts database, and every rating claim
was re-checked against the actual manufacturer datasheet rather than trusted from
zudo-pd's prose. One part failed that check — see
...
|-----|--------------------------|------------|
| `VBUS_IN` | `J1.A9 J1.B9 U1.VDD C1.2 C2.2 R14.1 R11.2 Q1.S J3.4 D5.cathode` | Receptacle VBUS (5 V pre-contract, 15 V post-contract). `U1.VDD` direct (operating range 4.1–22 V, absolute max 28 V — see [ratings matrix](./ratings-matrix.mdx)). `C1` 10 µF + `C2` 100 nF decoupling. `D5` cathode = SMAJ20A VBUS clamp |
...
| `GND` | `U1.GND U1.RESET U1.ADDR0 U1.ADDR1 U1.VSYS U1.EP J1.GND C1.1 C2.1 C30.1 C34.2 C35.2 R17.2(DNP) R18.2(DNP) J2.3 J3.5 D5.anode D6.2(DNP) D7.2(DNP) JOUT.5 JOUT.6` | Common ground. `U1.RESET` active-high, grounded = run. `U1.ADDR0/1` → I2C address `0x28`. `U1.VSYS` grounded (VDD-only supply). `U1.EP` = exposed thermal/electrical pad |
...
| Q1 | AO3401A | [C347476](https://jlcpcb.com/partdetail/C347476) | SOT-23 | Load switch (P-FET) | 862,120 |
...
| C2 | 100 nF 50V X7R | [C1711](https://jlcpcb.com/partdetail/C1711) | 0805 | VDD HF decouple | 17,509 |
...
| C35 | 100 nF 50V X7R | [C1711](https://jlcpcb.com/partdetail/C1711) | 0805 | Gate soft-start | 17,509 |
| R11 | 100 kΩ | [C25803](https://jlcpcb.com/partdetail/C25803) | 0603 | Gate pull-up | 7,990,119 |
| R12 | 56 kΩ | [C23206](https://jlcpcb.com/partdetail/C23206) | 0603 | Gate divider | 219,781 |
...
| JOUT | JST B6B-XH-A(LF)(SN) | [C144397](https://jlcpcb.com/partdetail/C144397) | THT, 2.5 mm pitch | Downstream interface connector | 47,935 |
...
- **Electrical limits to respect downstream:** anything wired to `VBUS_OUT` must
  tolerate the D5 (SMAJ20A) clamp ceiling during a fault event, not just the nominal
  15 V — see [Worst-Case Ratings Matrix](./ratings-matrix.mdx) for the exact numbers
  and which parts have residual (accepted, transient-only) margin gaps against that
  ceiling.
...
- [Worst-Case Ratings Matrix](./ratings-matrix.mdx) — per-part datasheet-sourced
  rating verification, including the USB-C receptacle swap.
