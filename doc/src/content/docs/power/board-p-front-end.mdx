...
| `Net-(Q1-G)` | `Q1.G R11.1 R12.2 C35.1` | Q1 gate. Generator evidence places `R11` 100 kΩ and `C35` 100 nF between `Q1_G` and `VBUS_IN`; this topology does not justify the obsolete gate-to-ground RC time-constant claim |
...
| `GND` | `U1.GND U1.RESET U1.ADDR0 U1.ADDR1 U1.VSYS U1.EP J1.GND C1.1 C2.1 C30.1 C34.2 R17.2(DNP) R18.2(DNP) J2.3 J3.5 D5.anode D6.2(DNP) D7.2(DNP) JOUT.5 JOUT.6` | Common ground. `U1.RESET` active-high, grounded = run. `U1.ADDR0/1` → I2C address `0x28`. `U1.VSYS` grounded (VDD-only supply). `U1.EP` = exposed thermal/electrical pad |
