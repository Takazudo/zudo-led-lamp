...
:::warning[PCB layout still needs this interface]
This change updates the schematic, generator and libraries. The existing
`board-l.kicad_pcb` has not been routed or updated for J5. Import the schematic
changes, place J5, route F1's output to J5.1 and J5.2 to V15, and remove any old
F1-to-V15 bypass. Run DRC and inspect the manufactured netlist before fabrication.
SW2 itself must never be imported as an on-board part.
:::
...
ON restarts the converters and resets the MCU. It does not bypass the existing
