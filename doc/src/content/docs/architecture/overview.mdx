...
description: The locked prototype architecture for the lamp — two lamp boards plus a separate SWD adapter, a 24-LED warm-white array at 500mA, an AL8860 constant-current buck, an STM32G031 control MCU, and an EC11 knob mapped to brightness. Decisions, not options.
sidebar_position: 10
---
...
| Power control | C496154 toggle assembled on rear Board L | Fused positive feed / switched V15 return; Board P remains powered |
...
The [C496154 toggle](./power-switch.mdx) turns off the LED driver and MCU supply
by opening Board L’s fused positive feed. Board P remains connected to USB-PD.
The toggle is assembled on the rear PCB with enclosure bushing support; ON restarts the MCU after sufficient rail decay; rapid toggles need verification.
