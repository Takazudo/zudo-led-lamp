...
uint8_t next_flicker_frame(uint8_t base_brightness, uint8_t depth, uint8_t smoothing /*0-255*/) {
    int16_t raw_noise = (int16_t)random_uniform(-128, 127);
    // one-pole low-pass: state += (raw - state) * alpha
    noise_state += ((raw_noise - noise_state) * smoothing) >> 8;
...
`smoothing` controls how "twitchy" (low value) vs. "lazy/breathing" (high value) the flicker feels — this is a natural knob-mapping target (see below). Layering 2-3 of these at different smoothing/depth settings and summing them (as used in more elaborate candle-emulation firmware) adds realism at the cost of a bit more CPU/RAM — worth prototyping both a single-layer and layered version.
...
- **Speed** — how fast the effect moves (`smoothing` in approach 1 inverted, `step_size`/`wander_chance` in approach 2, `loop_speed_q8` in approach 3). Turns a calm ember into an agitated flame.
