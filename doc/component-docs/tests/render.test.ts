...
describe("record page — structure", () => {
  ...
  it("states the owner is unknown rather than linking somewhere plausible but wrong", () => {
    // `ownerSkill` is an approved addition the adapter has not landed yet. Until
    // it does there is deliberately NO link: one pointing at the resource index
    ...
    assert.ok(driverPage.includes("The owning bundle is not identified in the published model"));
