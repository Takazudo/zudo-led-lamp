...
describe("the real circuit adapter", () => {
  it("runs the canonical Python validator and projects the corpus", async () => {
    ...
    assert.equal(result.report.records.available, 34);
    assert.equal(result.report.records.selected, 34);
    assert.equal(result.report.sources.available, 89);
    assert.equal(result.report.sources.selected, 89);
