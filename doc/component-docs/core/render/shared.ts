 ...
 *
 * Three facts bind a distributor identity as a structured object rather than a
 * scalar. Those render as their `key = value` pairs, in the key order the
 * adapter fixed. The flattening happens here, at the last possible moment and
 * for one table cell — the view model keeps the entries separate precisely so
 * that a surface with more room can lay them out as rows instead.
 */
export function factValue(fact: PublicFact): SafeText {
  const { value } = fact;
  // Narrowed by `typeof`, not `Array.isArray`: the latter's guard is `any[]`,
  // which does not subtract a `readonly` array from the union.
  if (typeof value === "number") return numberText(value, fact.factId);
      ...
      joinSafe(
        [
          ...
          typeof entry.value === "number"
            ? numberText(entry.value, fact.factId)
            : entry.value,
        ],
        " = ",
      ),
...
function numberText(value: number, factId: string): SafeText {
  return safeText(String(value), { field: `${factId} value` });
}
 ...
 *
 * This was a structural cast returning `null` while #60 waited on the view
 * model to carry the value. `PublicRecordIdentity.ownerSkill` is now a required
 * `SafeText` (#59), so the read is type-checked: if the field is ever removed,
 * this stops compiling instead of quietly falling back to the agent-resource
 * index and shipping a weaker link on every record page.
 *
 * The signature keeps its `| null` so the callers' fallback branches stay
 * intact — they are the right behaviour for any future provider whose records
 * genuinely have no owning bundle.
 */
