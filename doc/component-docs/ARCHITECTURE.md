...
Population ownership — **the types do not change when these land, only the arrays
stop being empty**:

| Field | Owner |
|---|---|
| `corpus`, `records[].identity` | Wave 1 (done) |
| `records[].aliases/sources/facts/coverage/interactions/pinMaps` | #59 |
| `integration` | #63 |
...
- a fact `value` keeps its JSON shape (`number | SafeText`) so `42` renders as `42`
  and units/conditions stay separate fields;
...
counts. Counts come from what the run actually did, not from what the matrix
