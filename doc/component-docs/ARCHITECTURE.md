...
`number | SafeText`. Four facts in the corpus record a distributor identity
...
- **An interaction is published on every record it names.** Nine of the 51
  interactions name multiple records, so 51 interactions land on 64 record pages;
...
is either absent or an empty `urlset`; one built page per record and no page the
projection did not produce; every record present in `search-index.json`.
...
- **Header nav `Claude` → `Components` is #61's change**, not Wave 1's. Wave 1
  ...
  way.
- **Sitemap stays disabled.** `sitemap` is not set in `zfb.config.ts`, so
  `@takazudo/zudo-doc` >=5.17.0 does not inject the `/sitemap.xml` route at all
  and `dist/` carries no sitemap. Before 5.17.0 the route was always injected and
  rendered an empty `urlset`, which is why `scan:artifacts` accepts both shapes.
  Verified unchanged after this work. Enabling it is a separate, tested decision.
...
```
python3 .claude/skills/component-spec-audit/scripts/validate.py     PASS: 34 lines; offline=True
