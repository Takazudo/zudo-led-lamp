...
async function main() {
  const recordDirectories = (await readdir(RECORDS_ROOT, { withFileTypes: true }))
    ...
    .sort();
  assert.equal(recordDirectories.length, 32, "built site must contain exactly 32 component record routes");

  const manifest = JSON.parse(await readFile(join(FOOTPRINT_ROOT, "manifest.json"), "utf8"));
  assert.equal(manifest.packages?.length, 22, "built footprint manifest must contain 22 packages");
  ...
  assert.equal(expectedFootprints.size, 22, "built footprint manifest names must be unique");
  ...
  assert.equal(referencedModels.size, 22, "record pages must deduplicate to exactly 22 selected WRLs");
  ...
  assert.equal(actualPreviewFiles.filter((path) => extname(path).toLowerCase() === ".svg").length, 22);
  assert.equal(actualPreviewFiles.filter((path) => extname(path).toLowerCase() === ".wrl").length, 22);
